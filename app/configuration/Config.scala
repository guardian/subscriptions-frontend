...
object Config {
 ...
 object Identity {
  ...
  def demoPromo(env: String) = {
    ...
    val promoCodes = (Seq(jellyFishPromoCode) /: (88 to 94))((seq, i) =>  seq :+ PromoCode(s"DGA$i") :+ PromoCode(s"DGB$i"))
  ...
  def discountPromo(env: String): Option[AnyPromotion] = {
    val prpIds = digipackRatePlanIds(env)
    val promoCodes = (Seq[PromoCode]() /: (13 to 26))((seq, i ) => seq :+ PromoCode(s"DPA$i") :+ PromoCode(s"DPB$i"))
    Some(Promotion(
